...
- 0x0000 - 0x0004: Зарезервировано (ничего нет).
...
- `arg` - некоторый 24-битный операнд из тела инструкции.
...
| `FADD`  | 0x0B | `a = DS.pop(); b = DS.pop(); DS.push(a + b)` | Игнорируется |
| `FSUB`  | 0x0C | `a = DS.pop(); b = DS.pop(); DS.push(a - b)` | Игнорируется |
| `FMUL`  | 0x0D | `a = DS.pop(); b = DS.pop(); DS.push(a * b)` | Игнорируется |
| `FDIV`  | 0x0E | `a = DS.pop(); b = DS.pop(); DS.push(a / b)` | Игнорируется |
...
2. **Построение AST (Parser)**: Строится дерево программы, учитывающее приоритеты математических операций.
